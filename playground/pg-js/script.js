...
function getRandomText(wordArray, wordCount = 20) {
    let text = '';
...
function highlightText() {
    ...
    sampleWords.forEach((word, index) => {
        if (index === currentWordIndex) {
            highlightedText += `<span class="current-word">${word}</span> `;
        } else {
...
function updateAccuracy(typedText) {
    ...
    highlightCurrentWord(typedText, currentWord);
}

function highlightCurrentWord(typedText, currentWord) {
    let highlightedWord = '';
    for (let i = 0; i < currentWord.length; i++) {
        if (i < typedText.length) {
            if (typedText[i] === currentWord[i]) {
                highlightedWord += `<span class="correct">${currentWord[i]}</span>`;
            } else {
                highlightedWord += `<span class="incorrect">${currentWord[i]}</span>`;
            }
        } else {
            highlightedWord += currentWord[i];
        }
    }
    sampleWords[currentWordIndex] = highlightedWord;
    highlightText();
}
...
function restartTest() {
    ...
    startTime = null;
    currentWordIndex = 0;
    sampleText = getRandomText(words);
